...
const EventsManagement = ({ theme }) => {
    ...
    const fetchEvents = async () => {
        try {
            ...
            // Fetch settings
            const settingsSnap = await get(ref(db, 'events/settings'));
            ...
            // Fetch all events
            const eventsSnap = await get(ref(db, 'events/eventsList'));

            if (eventsSnap.exists()) {
                const allEvents = Object.values(eventsSnap.val());
                ...
                const past = allEvents.filter(e => e.date < today).reverse(); // Most recent past first
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (editingEvent) {
                ...
                await update(ref(db, `events/eventsList/${editingEvent.id}`), updates);
                ...
                await set(ref(db, `events/eventsList/${eventId}`), newEvent);
    ...
    const handleDelete = async (eventId, isPastEvent) => {
        ...
        try {
            const db = getDatabase(app);
            await remove(ref(db, `events/eventsList/${eventId}`));
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
                    <div>
                        ...
                        <p className={`text-lg ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
                            Manage school events ({getTotalEvents()}/{maxEvents} events)
                        </p>
                    </div>
                                    ...
                                    <div className="p-6 space-y-6">
                                        ...
                                        <div className={`p-4 rounded-xl ${isDark ? 'bg-slate-900' : 'bg-slate-50'}`}>
                                            ...
                                            <p className={`text-xs mt-1 ${isDark ? 'text-slate-500' : 'text-slate-500'}`}>
                                                Total upcoming + past events combined
                                            </p>
                                        </div>
                                ...
                                >
                                    ...
                                    <form onSubmit={handleSubmit} className="p-6 space-y-6">
                                        ...
                                        <div>
                                            ...
                                            />
                                            <p className={`text-xs mt-1 ${isDark ? 'text-slate-500' : 'text-slate-500'}`}>
                                                Events automatically move to "Past Events" after this date
                                            </p>
                                        </div>
